    ...
    return (
      <div className="pt-1 p-4">
        ...
        { undefined === inquiries ? <div className="flex items-center h-full">
          ...
          <tbody>
            { inquiries.length ? inquiries.sort((a,b) => b.id - a.id).map(({ student, subject, message },i) => <tr key={i} className={`text-xs break-words px-4 py-4 ${i%2 ? 'bg-grey-lightest' : 'bg-grey-lighter'}`}>
