  ...
  async deleteCourse(e, course)
  {
    ...
    if ( deleted ) {
      ...
      this.props.setAlerts([{
        text: `Sandbox alert: data will be restored on next page reload.`,
        type: 'delete_course',
      }])
    } else {
      this.props.setAlerts([{
        text: `Error: course not deleted.`,
        type: 'delete_course',
      }])
    }
  }
        ...
        { undefined === courses ? <div className="flex items-center h-full">
          ...
          <tbody>
            { courses.length ? courses.sort((a,b) => a.id - b.id).map((course,i) => <tr key={i} className={`text-xs break-words px-4 py-4 ${i%2 ? 'bg-grey-lightest' : 'bg-grey-lighter'}`}>
              ...
              <td className="p-2">
                <Link to={`/admin/programs/${program.id}/courses/edit/${course.id}`} className="text-teal-dark cursor-pointer table">Edit Course</Link>
