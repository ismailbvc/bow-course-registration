...
class Login extends React.Component
{
  componentDidMount()
  {
    ...
    'Admin' == this.props.data_key && this.props.setAlerts([{
      text: <span>For admin login, may use these test credentials:<br/> - username: <code>admin</code>
        <br/> - password: <code>password</code></span>,
      type: 'login',
    }])
  }
  ...
  redirect()
  {
    ...
    return location.assign(next_url || '/')
  }
  ...
  async submit(e)
  {
    ...
    const { success, error } = await verifyLogin( username, password, this.props.data_key )
    ...
    if ( success ) {
      ...
      this.props.setAlerts([{
        text: `Error: ${error || 'invalid credentials.'}`,
        type: 'login',
      }])
    }
  }
