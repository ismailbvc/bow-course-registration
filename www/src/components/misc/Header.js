...
const UserIcon = ( { user, data_key, setAuthUser } ) => <React.Fragment>
  <small className="table ml-auto uppercase tracking-wide my-2">
    ...
    <a onClick={e =>
      {
        fetch(`/api/${data_key.toLowerCase()}/login`, { method: 'DELETE' })
        setAuthUser({ user: null })
      }} className="border cursor-pointer hover:bg-white hover:text-black px-2 rounded-sm text-white">Log out</a>
  </small>
</React.Fragment>