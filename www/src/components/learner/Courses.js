...
import { getProgramById, getProgramCourses, registerCourse, getCurrentTerm, getMyCourses } from './../../appdata'
...
export default class Courses extends React.Component
{
  async componentDidMount()
  {
    ...
    const program_id = +this.props.match.params.id
        ...
        , my_courses = courses.length ? await getMyCourses() : []
        , current_term = await getCurrentTerm()

    this.MOUNTED && this.setState({ program, courses, my_courses: my_courses.map(c => c.courseId), current_term,
  ...
  async register(e, { courseId: course_id, term })
  {
    e.preventDefault()

    const { loading, my_program, current_term, my_courses=[] } = this.state || {}
        , { user } = this.props.authLearner

    if ( ! my_program || current_term != term || -1 != my_courses.indexOf(course_id) )
      return
    ...
    const ok = await registerCourse(course_id)
  ...
  render()
  {
    const { courses, program, search='', current_term, my_program, loading, my_courses=[] } = this.state || {}
        , { user } = this.props.authLearner
        ...
        { undefined === courses ? <div className="flex items-center h-full">
          ...
          <tbody>
            { courses.length ? courses.sort((a,b) => a.id - b.id).map((course,i) => <tr key={i} className={`text-xs break-words px-4 py-4 ${i%2 ? 'bg-grey-lightest' : 'bg-grey-lighter'}`}>
              ...
              <td className="p-2">
                { !! my_program && course.term == current_term && -1 == my_courses.indexOf(course.courseId) && <button 
                ...
                { !! my_program && -1 != my_courses.indexOf(course.courseId) && <span>Registered</span> }
              </td>
            </tr>) : <tr>
