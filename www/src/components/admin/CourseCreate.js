...
export default class CoursesCreate extends React.Component
{
  async componentDidMount()
  {
    ...
    if ( edit_id ) {
      if ( ! course ) {
        ...
        return this.props.history.push(`/admin/programs/${program.id}/courses`)
      } else {
  ...
  async submit(e)
  {
    ...
    if ( inserted ) {
      ...
      this.props.setAlerts([{ text: `Error occurred, could not ${edit_id ? 'update' : 'insert new'} course.`, type: 'create_course' }])
